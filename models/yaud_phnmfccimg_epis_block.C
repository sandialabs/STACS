...
class YaudPhnMFCCImgEpisBlockPort : public yarp::os::BufferedPort<yarp::sig::Sound> {
  private:
    // Sound samples
    real_t samplefreq;
  ...
  public:
    ...
    YaudPhnMFCCImgEpisBlockPort(int nm);
...
YaudPhnMFCCImgEpisBlockPort::YaudPhnMFCCImgEpisBlockPort(int nm) {
  // Set up parameters
  // Sound samples
...
class YaudPhnMFCCImgEpisBlock : public ModelTmpl < 108, YaudPhnMFCCImgEpisBlock > {
  ...
  private:
#ifdef STACS_WITH_YARP
    YaudPhnMFCCImgEpisBlockPort* port;
...
// Renew model
//
void YaudPhnMFCCImgEpisBlock::Renew(std::vector<real_t>& state, std::vector<tick_t>& stick) {
#ifdef STACS_WITH_YARP
...
// Simulation step
//
tick_t YaudPhnMFCCImgEpisBlock::Step(tick_t tdrift, tick_t tdiff, std::vector<real_t>& state, std::vector<tick_t>& stick, std::vector<event_t>& events) {
#ifdef STACS_WITH_YARP
...
// Open ports
//
void YaudPhnMFCCImgEpisBlock::OpenPorts() {
#ifdef STACS_WITH_YARP
  std::string recv = portname[0] + "/recv";
  std::string request = portname[0] + "/request";
  port = new YaudPhnMFCCImgEpisBlockPort((int) param[0]);
...
// Close ports
//
void YaudPhnMFCCImgEpisBlock::ClosePorts() {
#ifdef STACS_WITH_YARP
  port->close();
  yarp::os::Bottle& b = preq->prepare();
  b.clear();
