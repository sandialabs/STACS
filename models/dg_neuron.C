...
class DGIzhiNeuron : public ModelTmpl < 60, DGIzhiNeuron > {
  public:
    ...
    DGIzhiNeuron() {
      ...
      statelist[0] = "v";
      statelist[1] = "u";
      statelist[2] = "I_syn_glu";
      statelist[3] = "I_syn_gaba";
      // sticks
      sticklist.resize(0);
      // auxiliary states
      auxstate.resize(0);
...
tick_t DGIzhiNeuron::Step(tick_t tdrift, tick_t tdiff, std::vector<real_t>& state, std::vector<tick_t>& stick, std::vector<event_t>& events) {
  ...
  state[0] = state[0] + (tstep/2)*(k*(state[0] - param[1])*(state[0] - param[0]) - state[1] + state[2] + state[3])/param[2];
  state[0] = state[0] + (tstep-tstep/2)*(k*(state[0] - param[1])*(state[0] - param[0]) - state[1] + state[2] + state[3])/param[2];
