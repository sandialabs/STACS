...
class YimgMnistEpisPort : public yarp::os::BufferedPort<yarp::sig::ImageOf<yarp::sig::PixelMono> > {
  public:
    std::vector<real_t> imgs;
    YimgMnistEpisPort(int np);
...
YimgMnistEpisPort::YimgMnistEpisPort(int np) {
...
// Rerun model
//
void YimgMnistEpis::Rerun(std::vector<real_t>& state, std::vector<tick_t>& stick) {
#ifdef STACS_WITH_YARP
...
// Simulation step
//
tick_t YimgMnistEpis::Step(tick_t tdrift, tick_t tdiff, std::vector<real_t>& state, std::vector<tick_t>& stick, std::vector<event_t>& events) {
#ifdef STACS_WITH_YARP
