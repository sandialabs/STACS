...
// Reset model
//
void LIFNeuronClamp::Reset(std::vector<real_t>& state, std::vector<tick_t>& stick) {
    state[0] = 0;
    state[1] = 0;
    state[2] = 0;
    state[3] = 0;
...
// Simulation step
//
tick_t LIFNeuronClamp::Step(tick_t tdrift, tick_t tdiff, std::vector<real_t>& state, std::vector<tick_t>& stick, std::vector<event_t>& events) {
  ...
  if (param[1] == 0.0) {
    state[0] = state[0] + (tstep/param[2])*(state[1] + state[2]);
  }
  else {
...
// Simulation jump
//
void LIFNeuronClamp::Jump(const event_t& event, std::vector<std::vector<real_t>>& state, std::vector<std::vector<tick_t>>& stick, const std::vector<auxidx_t>& auxidx) {
