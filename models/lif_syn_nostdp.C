...
* Class declaration
**************************************************************************/
class LIFSynNoSTDP : public ModelTmpl < ModelHash("lif_syn_nostdp"), LIFSynNoSTDP > {
  public:
    /* Constructor */
    LIFSynNoSTDP() {
      // parameters
...
// Simulation step
//
tick_t LIFSynNoSTDP::Step(tick_t tdrift, tick_t tdiff, std::vector<real_t>& state, std::vector<tick_t>& stick, std::vector<event_t>& events) {
  return tdiff;
}

// Simulation jump
//
void LIFSynNoSTDP::Jump(const event_t& event, std::vector<std::vector<real_t>>& state, std::vector<std::vector<tick_t>>& stick, const std::vector<auxidx_t>& auxidx) {
